...
function connect(callback) {
    ...
    request.get(adapter.config.url + '/items?recursive=false', auth, function (err, resp, body) {
        if (body) {
            ...
            try {
                ...
                syncObjects(objs, function () {
                    ...
                    if (adapter.config.username) {
                        adapter.config.url = adapter.config.url.replace(/^http:\/\//, 'http://' + adapter.config.username + ':' + adapter.config.password + '@');
                        adapter.config.url = adapter.config.url.replace(/^https:\/\//, 'https://' + adapter.config.username + ':' + adapter.config.password + '@');
                    }
