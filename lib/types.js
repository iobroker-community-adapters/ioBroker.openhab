...
const openHabTypes = {
    ...
    Number: item => {
        ...
        */
    	...
    	// A new feature of OH: type can be Number:Temperature => Temperature is used instead of the category information
        let itemTypeArray = item.type.split(':');
        let categoryInfo = item.category;
        if (itemTypeArray.length > 1) {
        	categoryInfo = itemTypeArray[1];
        }
        ...
        if (categoryInfo === 'temperature' || categoryInfo === 'Temperature') {
            if (data.write) {
                data.role = 'level.temperature'; // writable temperatures should be marked as level.
            } else {
                data.role = 'value.temperature';
            }
        } else if (categoryInfo === 'humidity' || categoryInfo === 'Humidity') {
